...
| `EMAIL_VERIFICATION_TOKEN_SECRET` | ✅ | — | **Yes** | Signs `GET /verify-email` links (time-bounded, 72h — audit fix M-4). **Fail-closed**: unset ⇒ no verification link can be minted **and** none verifies, so every account is stuck at `/verify-email`. `openssl rand -base64 32`, dedicated (never reuse `AUTH_SECRET`). |
| `UNSUBSCRIBE_TOKEN_SECRET` | ✅ | — | **Yes** | Signs `GET /unsubscribe` links (time-bounded, 90 days — audit fix M-4). **Fail-closed**. `openssl rand -base64 32`, dedicated. |
