...
beforeEach(async () => prisma.$executeRawUnsafe(`TRUNCATE "Session","Account","User" RESTART IDENTITY CASCADE`));
...
describe("searchUsers plan/role filters and sort", () => {
  ...
  it("filters by role", async () => {
    await prisma.user.create({ data: { email: `${randomUUID()}@example.com`, role: "USER" } });
    await prisma.user.create({ data: { email: `${randomUUID()}@example.com`, role: "MANAGER" } });
  ...
  it("combines email, plan, and role filters (AND, not OR)", async () => {
    const target = await prisma.user.create({ data: { email: "target-user@example.com", plan: "BASIC", role: "SUPPORT_ADMIN" } });
    await prisma.user.create({ data: { email: "target-other-plan@example.com", plan: "FREE", role: "SUPPORT_ADMIN" } });
  ...
  it("defaults to createdAt descending, and sort=createdAt_asc reverses it", async () => {
    const older = await prisma.user.create({ data: { email: `${randomUUID()}@example.com` } });
    await prisma.user.update({ where: { id: older.id }, data: { createdAt: new Date("2026-01-01T00:00:00Z") } });
    const newer = await prisma.user.create({ data: { email: `${randomUUID()}@example.com` } });
