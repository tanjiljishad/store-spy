...
describe("verifyCredentials", () => {
  it("succeeds with the right email/password", async () => {
    ...
    await prisma.user.create({ data: { email, passwordHash: await hashPassword("correct-password") } });
  ...
  it("returns null for an OAuth-only user (no passwordHash) — never crashes on a null hash", async () => {
    const email = `${randomUUID()}@example.com`;
    await prisma.user.create({ data: { email } }); // no passwordHash — as if created via Google sign-in
